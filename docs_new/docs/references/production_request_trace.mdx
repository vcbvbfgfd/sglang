...
The span reports request and sampling configuration, prompt/completion/reasoning
token usage, KV cache hit and miss tokens, device/host/storage cache breakdown,
TTFT, TPOT, end-to-end and scheduler latency, request ID, weight version, DP rank,
finish reasons, retractions, and speculative decoding acceptance statistics when
those values are available.

Configure the request trace and metrics exporters with standard OTLP variables:
...
The metrics export interval defaults to 30 seconds. It can be overridden with
`SGLANG_OTEL_METRICS_EXPORT_INTERVAL_MILLIS` or `OTEL_METRIC_EXPORT_INTERVAL`.

KV cache metrics include `sglang.kv_cache.lookup_tokens`,
`sglang.kv_cache.hit_tokens`, `sglang.kv_cache.miss_tokens`, and
`sglang.kv_cache.request_hit_ratio`. Compute an aggregate hit ratio by dividing
the rate of hit-token counters by the rate of lookup-token counters; do not average
per-request ratios because prompt lengths differ.

Speculative decoding metrics include `sglang.speculative.accept_ratio`, accepted
and proposed draft-token counters, and verification-call counters. For an
aggregate acceptance ratio, divide accepted draft-token rate by proposed
draft-token rate.

Prompt and completion content is excluded by default. Enable it only when the
deployment's privacy policy allows it:
...
The internal SGLang request root span also carries KV cache source breakdown,
retractions, DP rank, and speculative decoding statistics for non-OpenAI paths.
